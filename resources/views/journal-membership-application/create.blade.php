...
@section('content')
    ...
    <form action="{{ route('membership-applications.store') }}" method="POST">
        @csrf
        
        <div class="card mb-4">
            <div class="card-header">
                <h5>1. Journal Application</h5>
            </div>
            <div class="card-body row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="requested_role" class="form-control" required>
                        ...
                        <option value="owner" {{ old('requested_role') == 'owner' ? 'selected' : '' }}>Owner</option>
                        <option value="editor" {{ old('requested_role') == 'editor' ? 'selected' : '' }}>Editor</option>
                        <option value="reviewer" {{ old('requested_role') == 'reviewer' ? 'selected' : '' }}>Reviewer</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <button type="submit" class="btn btn-primary">Save & Continue to Evidence Upload</button>
    </form>
            </div>
        </div>
    </div>
